...
int main(int argc, char* argv[])
{
    ...
    auto vision_sender = std::make_unique<ThreadedProtoUdpSender<TbotsProto::Vision>>(
        std::string(MULTICAST_CHANNELS[0]) + "%" + std::string(argv[1]), VISION_PORT,
        true);

    auto primitive_sender =
        std::make_unique<ThreadedProtoUdpSender<TbotsProto::Primitive>>(
            std::string(MULTICAST_CHANNELS[0]) + "%" + std::string(argv[1]),
            PRIMITIVE_PORT, true);

    auto status_listener =
        std::make_unique<ThreadedProtoUdpListener<TbotsProto::RobotStatus>>(
            std::string(MULTICAST_CHANNELS[0]) + "%" + std::string(argv[1]),
            ROBOT_STATUS_PORT, std::function(callback), true);
