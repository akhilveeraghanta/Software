...
{
   public:
    explicit AngleSegment(double angle_max, double angle_min, double cartesian_angle_max,
                          double cartesian_angle_min)
        : angle_max_(angle_max),
          angle_min_(angle_min),
          cartesian_angle_max_(cartesian_angle_max),
          cartesian_angle_min_(cartesian_angle_min)
    {
    }
...
{
   public:
    explicit AngleMap(double angle_max, double angle_min, double angle_0_max,
                      double angle_0_min)
        : angle_max(angle_max),
          angle_min(angle_min),
          angle_0_max(angle_0_max),
          angle_0_min(angle_0_min)
    {
    }
    ...
    void addNonViableAngleSegment(AngleSegment &angle_seg)
    {
        ...
        {
            if ((angle_seg.getAngleMax() < taken_angle_seg.getAngleMin() &&
                 angle_seg.getAngleMin() > taken_angle_seg.getAngleMax()) ||
                (angle_seg.getAngleMin() > taken_angle_seg.getAngleMax() &&
                 angle_seg.getAngleMin() < taken_angle_seg.getAngleMin()))
            {
                if (taken_angle_seg.getAngleMax() > angle_seg.getAngleMax())
                {
                    taken_angle_seg.setCartesianAngleMax(
                        angle_seg.getCartesianAngleMax());
                }
                taken_angle_seg.setAngleMax(
                    std::min(taken_angle_seg.getAngleMax(), angle_seg.getAngleMax()));

                if (taken_angle_seg.getAngleMin() < angle_seg.getAngleMin())
                {
                    taken_angle_seg.setCartesianAngleMin(
                        angle_seg.getCartesianAngleMin());
                }
                taken_angle_seg.setAngleMin(
                    std::max(taken_angle_seg.getAngleMin(), angle_seg.getAngleMin()));
                return;
            }
        }
    ...
    AngleSegment getBiggestViableAngleSegment()
    {
        ...
        if (this->taken_angle_segments.empty())
        {
            biggest_viable_angle_seg = AngleSegment(this->angle_max, this->angle_min,
                                                    this->angle_0_max, this->angle_0_min);
        ...
        {
            biggest_viable_angle_seg = AngleSegment(
                this->angle_max, first_taken_angle_seg.getAngleMax(), this->angle_0_max,
                first_taken_angle_seg.getCartesianAngleMax());
        }
        else if (first_taken_angle_seg.getAngleMax() < this->angle_max &&
                 first_taken_angle_seg.getAngleMin() > this->angle_min)
        {
            return biggest_viable_angle_seg;
        }
        ...
        if (last_taken_angle_seg.getAngleMin() < this->angle_min)
        {
            AngleSegment viable_angle_seg = AngleSegment(
                last_taken_angle_seg.getAngleMin(), this->angle_min,
                last_taken_angle_seg.getCartesianAngleMin(), this->angle_0_min);
        ...
        //        std::sort(obstacles.begin(), obstacles.end(), [&field](const Segment &a,
        //        const Segment &b) -> int {
             ...
             i < this->taken_angle_segments.end() - 1; i++)
        {
            ...
            AngleSegment viable_angle_seg = AngleSegment(
                taken_angle_seg.getAngleMin(), next_taken_angle_seg.getAngleMax(),
                taken_angle_seg.getCartesianAngleMin(),
                next_taken_angle_seg.getCartesianAngleMax());
...
std::pair<double, double> cartesianValues(const Field &field, double screen_x,
                                          double screen_y);
 ...
 */
std::optional<Shot> calcBestShotOnGoalExp(const Segment &goal_post,
 ...
 */
std::optional<Shot> calcBestShotOnGoalExp(
    const Field &field, const Team &friendly_team, const Team &enemy_team,
    const Point &shot_origin, TeamType goal,
    const std::vector<Robot> &robots_to_ignore = {});

std::pair<AngleSegment, std::pair<AngleMap, std::vector<Segment>>>
calcBestShotOnGoalExpTest(const Field &field, const Team &friendly_team,
                          const Team &enemy_team, const Point &shot_origin, TeamType goal,
                          const std::vector<Robot> &robots_to_ignore);
