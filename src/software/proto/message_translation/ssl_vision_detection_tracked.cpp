...
std::unique_ptr<SSLProto::TrackedBall> createSSLTrackedBall(const BallState& ball)
{
    ...
    tracked_ball->set_allocated_pos(
        createVector3(ball.position().toVector(), 0).release());
    ...
    const RobotStateWithId& robot, SSLProto::Team team)
{
    ...
    tracked_robot->set_allocated_pos(
        createVector2(robot.robot_state.position().toVector()).release());
    tracked_robot->set_allocated_vel(
        createVector2(robot.robot_state.velocity()).release());
    ...
    const std::vector<RobotStateWithId>& blue_robots)
{
    ...
    for (const auto& yellow_robot : yellow_robots)
    {
        *(tracked_frame->add_robots()) =
            *createSSLTrackedRobot(yellow_robot, SSLProto::Team::YELLOW);
    }
